<x-layout>
    <div class="mw-md">
        ...
        <form action="{{ route('deleteClass', ['id' => $classroom->id]) }}" method="post" class="d-flex justify-content-end">
            @csrf
            @method('DELETE')
            <button onclick="return confirm('Yakin untuk dihapus?')" class="btn btn-outline-dark mt-2">
                Delete Class
            </button>
        </form>
    </div>
