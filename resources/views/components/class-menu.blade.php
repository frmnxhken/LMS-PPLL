<div class="col-md-4 mb-3">
    <ul class="list-group w-100 d-flex flex-row flex-md-column">
        ...
        @else
        <form action="{{ route('outClass', ['id' => $id]) }}" method="post">
            @csrf
            @method('DELETE')
            <button onclick="return confirm('Yakin keluar?')" class="list-group-item flex-fill w-100 text-center text-md-start fs-vs">Keluar</button>
        </form>
        @endif
    </ul>
</div>